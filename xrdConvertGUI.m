...
lbFiles = uilistbox(mainGL, 'Multiselect', 'on', ...
    'Tooltip', 'XRD files to convert — Ctrl+click or Shift+click for multi-select');
...
btnSelectAll = uibutton(mainGL, 'push', 'Text', 'Select All', ...
    ...
    'Tooltip', 'Select all files in the list for conversion');
...
btnDeselectAll = uibutton(mainGL, 'push', 'Text', 'Deselect All', ...
    ...
    'Tooltip', 'Deselect all files (no files will be converted)');
...
ddFormat = uidropdown(mainGL, ...
    ...
    'Tooltip', ['Standard CSV: plain comma-separated file readable by any software. ' ...
                'Origin ASCII: tab-separated with Origin-compatible headers. ' ...
                'Send to Origin: imports directly into a running Origin session — ' ...
                'Origin must be open before converting.']);
...
ddIntensity = uidropdown(mainGL, ...
    ...
    'Tooltip', ['Both: write a counts-per-second column and a raw-counts column. ' ...
                'CPS only: normalised intensity only (divides by scan time). ' ...
                'Counts only: raw detector counts with no time normalisation.']);
...
ddOutputLoc = uidropdown(mainGL, ...
    ...
    'Tooltip', ['Same folder as source: converted files are saved next to the original XRD file. ' ...
                'Custom folder: choose a single output directory for all converted files.']);
...
btnBrowseOutput = uibutton(mainGL, 'push', 'Text', char(8230), ...
    'ButtonPushedFcn', @onBrowseOutputDir, ...
    'Tooltip', 'Browse for the custom output folder');
...
cbMetadata = uicheckbox(mainGL, 'Text', 'Include metadata header', 'Value', true, ...
    'Tooltip', 'Prepend scan metadata (wavelength, generator settings, date) as comment lines in the output file');
...
btnConvert = uibutton(mainGL, 'push', 'Text', 'Convert', ...
    ...
    'Tooltip', 'Convert all selected files using the options above');
