...
%                   .onWriteXRDcsv(src, evt)
    ...
    appData.plotOptFig = uifigure('Name', 'Plot Options', ...
        'Position', [figPos(1) + 200, figPos(2) + figPos(4) - 350, 220, 310], ...
        'Resize', 'off', ...
        'CloseRequestFcn', @(~,~) bosonPlotter.closePlotOptMenu(appData), ...
        'KeyPressFcn', @(~,evt) localKey(appData, evt));

    poGL = uigridlayout(appData.plotOptFig, [12 1], ...
        'RowHeight', {16, 26, 26, 26, 5, 16, 26, 26, 5, 16, 26, 26}, ...
        'ColumnWidth', {'1x'}, ...
    ...
    lblLbl = uilabel(poGL, 'Text', 'LABELS', 'FontSize', 9, ...
        'FontWeight', 'bold', 'FontColor', HDR_FC);
    lblLbl.Layout.Row = 10;

    localBtn(poGL, 11, 'Edit Legend Labels', callbacks.onEditLegend, ...
        'Bulk-edit all legend labels: per-row text, find/replace, strip common prefix, fill from metadata', ...
        appData, BTN_BG, BTN_FC);
