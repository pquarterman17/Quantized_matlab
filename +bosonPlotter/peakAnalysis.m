...
ctrlGL = uigridlayout(ctrlPanel, [26 2], ...
    'RowHeight', {18, 26, 26, 8, ...         % Background section
                  ...
                  34, 34, 34, 8, ...          % Actions: Detect, FitAll, Pawley, spacer
                  34, 34, 'fit'}, ...
    'ColumnWidth', {100, '1x'}, ...
...
row = row + 1;
btnPawley = uibutton(ctrlGL, 'Text', 'Refine cell (Pawley)', ...
    'BackgroundColor', [0.35 0.25 0.55], 'FontColor', BTN_FG, ...
    'Tooltip', ['Whole-pattern Pawley refinement of lattice parameters ' ...
                'using the detected peaks'' 2θ positions.'], ...
    'ButtonPushedFcn', @(~,~) onPawleyRefine());
btnPawley.Layout.Row = row; btnPawley.Layout.Column = [1 2];
...
% ════════════════════════════════════════════════════════════════════════
    ...
    function onPawleyRefine()
    %ONPAWLEYREFINE  Refine unit-cell parameters using the detected 2θ peaks.
    %   Prompts the user for an initial (a, b, c, α, β, γ) cell + Bravais
    ...
    %   intensity pattern. Reports refined cell and Rwp in a modal dialog.
        ...
        % Prompt user for initial cell + symmetry. Default values are a
        % cubic seed using the lowest-2θ peak's d-spacing (CuKα1) as a
        % crude guess.
        defAns = inputdlg( ...
            {'a (Å)', 'b (Å)', 'c (Å)', 'α (°)', 'β (°)', 'γ (°)', ...
             'Centering (P/I/F/A/B/C/R)', 'Wavelength (Å)', 'Max hkl'}, ...
            'Pawley initial cell', 1, ...
            {'5.43', '5.43', '5.43', '90', '90', '90', 'F', '1.5406', '6'});
        if isempty(defAns), return; end
        try
            phaseInfo = struct( ...
                'a',        str2double(defAns{1}), ...
                'b',        str2double(defAns{2}), ...
                'c',        str2double(defAns{3}), ...
                'alpha',    str2double(defAns{4}), ...
                'beta',     str2double(defAns{5}), ...
                'gamma',    str2double(defAns{6}), ...
                'symmetry', strtrim(defAns{7}), ...
        ...
        try
            result = fitting.pawleyRefine(xv, yv, phaseInfo, ...
                Wavelength=lambda, MaxTwoTheta=max(xv));
            ...
            msg = sprintf([ ...
                ...
                '   c = %.5f Å  (was %.5f)\n' ...
                '   α = %.3f°   β = %.3f°   γ = %.3f°\n\n' ...
                'Rwp = %.4f    Peaks matched = %d'], ...
                result.cell(1), result.cellInitial(1), ...
                result.cell(2), result.cellInitial(2), ...
            ...
            hold(axMain, 'on');
            plot(axMain, xv, result.model, '-', ...
                'Color', [1.0 0.6 0.2], 'LineWidth', 1.0, ...
                'DisplayName', 'Pawley model');
            hold(axMain, 'off');

            hFig.Pointer = 'arrow';
            setStatusLocal(sprintf( ...
                'Pawley: a=%.4f Å, Rwp=%.4f, %d peaks', ...
                result.cell(1), result.rwp, result.nPeaks));
            options.StatusFcn(sprintf('Pawley refined cell: a=%.4f Å, Rwp=%.4f', ...
