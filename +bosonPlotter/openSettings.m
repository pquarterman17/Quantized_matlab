...
%                 .onCustomiseToolbar()
    ...
    % Read the *preference* (which may be 'Auto') so the dropdown
    % round-trips correctly. If themePref isn't on appData yet (older
    % session), fall back to the resolved theme.
    if isprop(appData, 'themePref') && ~isempty(appData.themePref)
        themePrefValue = appData.themePref;
    else
        themePrefValue = appData.theme;
    end
    ddThemeDlg = uidropdown(sGL, ...
        'Items', {'Light','Dark','Auto'}, ...
        'Value', themePrefValue, ...
        'FontSize', 12, ...
        'Tooltip', 'Auto follows your OS appearance (R2025a+ uses MATLAB''s system theme; falls back to OS detection on older releases).', ...
