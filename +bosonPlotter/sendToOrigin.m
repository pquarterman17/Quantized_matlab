...
%                  .originObj        (optional) injected COM handle (tests)
    ...
    weOwn = false;
    injected = isfield(guiState, 'originObj') && ~isempty(guiState.originObj);
    % Trace every COM step for REAL sends (so the user has error codes to note);
    % stay quiet when a mock is injected (tests) unless explicitly asked.
    diagnostic = ~injected || (isfield(guiState, 'diagnostic') && guiState.diagnostic);
    if injected
    ...
    if diagnostic
        utilities.logError('sendToOrigin:diag', ...
            sprintf('send start: mode=%s, %d dataset(s)', mode, numel(selIdx)));
    end
    ...
    elseif nOk > 0
        bosonPlotter.quietAlert(fig, ...
            sprintf('Sent %d of %d datasets; %d failed.\nDetails (error codes) logged to:\n%s', ...
                nOk, numel(selIdx), numel(selIdx) - nOk, utilities.errorLogPath()), ...
            'Origin Export');
    else
        originFailedAlert(fig);
    end
...
function originFailedAlert(fig)
    bosonPlotter.quietAlert(fig, ...
        ['Origin export failed. Origin may be busy or the COM call was ' ...
         'rejected.' newline 'Error codes logged to:' newline utilities.errorLogPath()], ...
        'Origin Export');
end
