...
%   Boson.m: all mutable state lives in the appData struct, all
...
% ── Programmatic API ──────────────────────────────────────────────────────
%
%   api = Fermion() returns a struct of function handles:
%
...
%   Boson, test_em_gui_harness
    ...
    % Row 16: separator
    % Row 17-18: Measurement Statistics / Batch Measurement / Export to Boson
    ...
    btnExportToDP = uibutton(measureInnerGL, 'Text', 'Profile → Boson', ...
        'ButtonPushedFcn', @onExportProfileToDP, ...
        'BackgroundColor', BTN_EXPORT, ...
        'FontColor', BTN_FG, ...
        'Enable', 'off', ...
        'Tooltip', 'Send last line profile to Boson as standard data struct');
    ...
    function varargout = stubNotImplemented(funcName)
        warning('Fermion:notImplemented', ...
    ...
    %SETCONTRASTAPI  Set Low/High contrast sliders and refresh display.
        if isempty(appData.filteredPixels)
            warning('Fermion:noImage', 'No image loaded.');
            return;
        end
        ...
        if lo >= hi
            warning('Fermion:invalidContrast', ...
    ...
    function onExportProfile(~, ~)
        ...
        try
            fid = fopen(outPath, 'w');
            if fid == -1
                error('Fermion:exportFailed', 'Cannot open file for writing: %s', outPath);
            end
            fprintf(fid, '%s\n', header);
            fclose(fid);
    ...
    %SETEDSCHANNELAPI  Programmatic setter for EDS channel properties.
        if idx < 1 || idx > numel(appData.edsChannels)
            error('Fermion:invalidIdx', 'Channel index %d out of range', idx);
        end
        switch field
            case 'color'
            ...
            otherwise
                error('Fermion:invalidField', 'Unknown field: %s', field);
        end
        refreshEDSList();
        if appData.edsMode, compositeEDS(); end
    end
    ...
    %  Batch Measurement, Export to Boson
    % ════════════════════════════════════════════════════════════════════
    ...
    %ON3DSURFACE  Render the current image as a 3D height map.
        ...
        surfFig = figure('Name', 'EM 3D Surface View', 'NumberTitle', 'off', ...
            'Units', 'pixels', 'Position', [200 150 700 550], 'Tag', 'fermion3D');
        surfAx = axes(surfFig);
    ...
    %ONMEASUREMENTSTATS  Show aggregate statistics for all measurements.
        ...
        statsFig = figure('Name', 'Measurement Statistics', 'NumberTitle', 'off', ...
            'Units', 'pixels', 'Position', [300 200 500 400], 'Tag', 'fermionMeasStats');
        subplot(2, 1, 1);
    ...
    %ONEXPORTPROFILETODP  Send last line profile to Boson as data struct.
        if isempty(appData.lastProfile.dist)
        ...
        data = parser.createDataStruct( ...
            ...
            struct('source', 'Fermion line profile', 'parserName', 'Fermion'));
        % Save to workspace and launch Boson
        assignin('base', 'emProfileData', data);
        setStatus('Line profile exported to workspace as ''emProfileData''. Launch Boson to load.');
        try
            Boson;
        catch
            % Boson may not be on path
        end
    end
    ...
    %RENAMEBATCH  Rename files on disk with baseName_001, _002, ... pattern.
        ...
        for ri = 1:numel(idxs)
            ...
            try
                ...
                warning('Fermion:rename', 'Failed to rename %s: %s', ...
                    srcPath, ME.message);
            end
        end
