...
function appData = doCaptureClick(appData, ctx)
    ...
    hMark = line(ctx.ax, x, y, ...
        'Marker',           'o', ...
        'MarkerSize',       6, ...
        'Color',            ctx.overlayColor, ...
        'MarkerFaceColor',  ctx.overlayColor, ...
        'LineStyle',        'none', ...
        'HandleVisibility', 'off');
...
function appData = doExecuteRectROI(xMin, xMax, yMin, yMax, appData, ctx)
    ...
    areaStr = sprintf('%d px\xb2', roiArea);
    if appData.activeIdx >= 1
        ...
        if imgInfo.calibrated && ~isnan(imgInfo.pixelSize)
            ...
            areaStr = sprintf('%.4g %s\xb2', calibArea, imgInfo.pixelUnit);
        end
    end
    ...
    if isempty(measClr), measClr = ctx.overlayColor; end
