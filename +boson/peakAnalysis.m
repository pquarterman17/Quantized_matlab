...
%
%   boson.peakAnalysis(datasets, activeIdx, mainAx)
%
...
%
%   Results are communicated back to Boson via PeakUpdateCallback.
...
if isempty(datasets) || activeIdx < 1 || activeIdx > numel(datasets)
    error('boson:peakAnalysis:noDataset', 'No valid dataset.');
end
...
if numel(xv) < 10
    error('boson:peakAnalysis:tooFewPoints', 'Need at least 10 data points.');
end
...
row = 22;
btnApply = uibutton(ctrlGL, 'Text', 'Apply to Boson', ...
...
% ════════════════════════════════════════════════════════════════════════
    ...
    function onApply()
        ...
        setStatusLocal(sprintf('Applied %d peaks to Boson.', numel(peaksOut)));
