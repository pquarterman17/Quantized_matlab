...
% ── Syntax ────────────────────────────────────────────────────────────────
%
%   fig = boson.spreadsheetPopup(dataStruct)
%   fig = boson.spreadsheetPopup(dataStruct, Title=..., OnEdit=..., ReadOnly=...)
%
% ── Inputs ────────────────────────────────────────────────────────────────
%
...
%   boson.spreadsheetPopup(d);
%   boson.spreadsheetPopup(d, Title='My Dataset', ReadOnly=true);
%
% ════════════════════════════════════════════════════════════════════════
...
% ════════════════════════════════════════════════════════════════════════
    ...
    %ONFILTER  Apply the filter expression from the edit field.
        ...
        try
            mask = boson.filterRows(dataStruct, expr);
    ...
    %ONCELLEDIT  Propagate cell edits through OnEdit callback.
        ...
        if ~isempty(Options.OnEdit) && isa(Options.OnEdit, 'function_handle')
            try
                ...
                warning('boson:spreadsheetPopup:editCallbackError', ...
