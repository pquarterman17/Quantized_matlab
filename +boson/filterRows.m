...
%
%   Syntax:
%     mask = boson.filterRows(dataStruct, expression)
%
%   Inputs:
...
%
%   Examples:
%     mask = boson.filterRows(d, 'Temp > 300')
%     mask = boson.filterRows(d, 'Field >= -500 & Field <= 500')
%     mask = boson.filterRows(d, 'between(x, 0, 1e-3)')
%     mask = boson.filterRows(d, '')   % all-true mask
...
if pos <= numel(tokens)
    error('boson:filterRows:unexpectedToken', ...
...
% ════════════════════════════════════════════════════════════════════════
    ...
    function val = parseComparison()
        ...
        switch op
            ...
            case '~='
                val = lhs ~= rhs;
            otherwise
                error('boson:filterRows:unknownOp', ...
    ...
    function val = parsePrimary()
        if pos > numel(tokens)
            error('boson:filterRows:unexpectedEnd', ...
        ...
        if strcmp(tok, '(')
            ...
            val = parseOr();
            if pos > numel(tokens) || ~strcmp(tokens{pos}, ')')
                error('boson:filterRows:missingParen', ...
        ...
        error('boson:filterRows:unknownToken', ...
    ...
    function expectOpen()
        if pos > numel(tokens) || ~strcmp(tokens{pos}, '(')
            error('boson:filterRows:missingOpenParen', ...
    ...
    function expectClose()
        if pos > numel(tokens) || ~strcmp(tokens{pos}, ')')
            error('boson:filterRows:missingCloseParen', ...
    ...
    function expectComma()
        if pos > numel(tokens) || ~strcmp(tokens{pos}, ',')
            error('boson:filterRows:missingComma', ...
...
while i <= n
    ...
    error('boson:filterRows:badChar', ...
