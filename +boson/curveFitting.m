...
%
%   Syntax:
%       boson.curveFitting(datasets, activeIdx, mainAx)
%       boson.curveFitting(datasets, activeIdx, mainAx, 'StatusFcn', fcn)
%       boson.curveFitting(datasets, activeIdx, mainAx, 'ButtonColors', s)
%
%   Inputs:
%       datasets    cell array of dataset structs (each with .corrData / .data)
%       activeIdx   index into datasets of the currently active file
%       mainAx      axes handle of the main Boson plot (used by "Plot on Main"
...
%                       Default: standard Boson colours
%
%   Description:
...
if isempty(datasets) || activeIdx < 1 || activeIdx > numel(datasets)
    error('boson:curveFitting:noDataset', ...
...
if isempty(plotD) || isempty(plotD.time)
    error('boson:curveFitting:emptyData', ...
...
uibutton(cfBottomGL, 'Text', 'Plot on Main', ...
    'BackgroundColor', BTN_PRIMARY, 'FontColor', [1 1 1], ...
    'FontSize', 9, ...
    'Tooltip', 'Overlay fit curve on the main Boson axes', ...
    ...
    %RESOLVEMODEL  Get current model function, params, bounds from UI state.
        ...
        if strcmp(modelName, 'Custom Equation')
            if isempty(customFcn)
                error('boson:curveFitting:noCustom', ...
