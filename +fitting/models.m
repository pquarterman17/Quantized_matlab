...
catalog = [ ...
    ...
    % Pseudo-Voigt delegates to utilities.pseudoVoigt.  The width parameter
    % w is the Gaussian σ; FWHM = 2*sqrt(2*log(2))*σ ≈ 2.355*σ is the shared
    % full-width used for both the Lorentzian and Gaussian components inside
    % utilities.pseudoVoigt (which normalises both components so that the peak
    % height equals A regardless of η).  This replaces the previous inline
    % formula that treated w as both σ and γ simultaneously — that was
    % numerically inconsistent; the new form is the standard pseudo-Voigt
    % convention used in powder diffraction.  Existing fits with w > 0 and
    % η ∈ (0,1) will show a small numerical difference in the Lorentzian
    % contribution only.
    mdl('Pseudo-Voigt', 'Peak', 'y = η·L(x;FWHM) + (1-η)·G(x;σ), FWHM=2√(2ln2)·σ', ...
        @(x,p) utilities.pseudoVoigt(x(:)', p(2), 2*sqrt(2*log(2))*p(3), p(1), p(4))', ...
    ...
    % ── Magnetic ───────────────────────────────────────────────────────
    % IMPORTANT: Brillouin and Langevin below use the field `x` in Tesla.
    % The numerical constants 5.7884e-5 (μB in eV/T) and 8.617e-5 (kB in
    % eV/K) assume SI Tesla field input. VSM data is usually in Oersted;
    % pass H_tesla = H_Oe * 1e-4 before fitting or the fit will scale
    % the argument by 10^4 and return meaningless parameters. A future
    % version should accept a FieldUnit option.
    mdl('Langevin', 'Magnetic', 'y = A·(coth(x/B) - B/x)', ...
        @(x,p) langevinFcn(x, p), ...
        {'A','B'}, [1 1], [0 0], [INF INF])
    mdl('Brillouin', 'Magnetic', 'M = Ms·B_J(g·μB·J·B_T/(kB·T))  [B_T in Tesla]', ...
