...
**Total: 41 `.m` files implemented, 152 tests passing (85 + 67).**
...
| `DataPlotter.m` | ~11000+ | Same as emViewerGUI pattern. Largest GUI. Sub-dialogs (FFT thickness, lattice refinement, Williamson-Hall) are nested functions launching their own `uifigure`. |
...
1. **State management:** `appData` is a plain struct held in function closure.
   ...
   (emViewerGUI.m line 85-119, DataPlotter.m line 120-136)
...
├── materialsCalcGUI.m              # [Phase 3] Standalone GUI (~1500-2000 lines est.)
├── +calc/                          # Calculation modules
│   ├── constants.m                 # ✅ Physical constants struct (CODATA 2018, 14 constants)
│   ├── unitConvert.m               # ✅ Unit parsing + conversion engine (492 lines)
│   ├── elementData.m               # ✅ Periodic table data (118 elements, 18 properties)
│   ├── +crystal/                   # ✅ 10 functions — Crystal/lattice + epitaxy/strain
│   │   ├── dSpacing.m              #    d-spacing for general triclinic lattice
│   │   ├── twoThetaFromD.m         #    Bragg's law forward
│   │   ├── dFromTwoTheta.m         #    Bragg's law inverse
│   │   ├── unitCellVolume.m        #    General unit cell volume
│   │   ├── atomicDensity.m         #    Atoms per cm³ from lattice params
│   │   ├── densityFromMolar.m      #    Mass density from molar mass
│   │   ├── latticeMismatch.m       #    Film/substrate epitaxial mismatch
│   │   ├── criticalThickness.m     #    Matthews-Blakeslee critical thickness
│   │   ├── strainFromPoisson.m     #    Biaxial strain ↔ out-of-plane strain
│   │   └── tetragonalDistortion.m  #    c/a ratio and % distortion
│   ├── +electrical/                # ✅ 5 functions — Resistivity, conductivity, mobility
│   │   ├── resistivity.m
│   │   ├── conductivity.m
│   │   ├── mobility.m
│   │   ├── currentDensity.m
│   │   └── sheetResistance.m
│   ├── +semiconductor/             # ✅ 14 files — Carrier densities, Fermi level, transport
│   │   ├── materialPresets.m       #    8 material presets (Si, Ge, GaAs, InP, GaN, SiC, SiO2, Al2O3)
│   │   ├── intrinsicCarrierConc.m
│   │   ├── carrierConcentration.m
│   │   ├── fermiLevel.m
│   │   ├── debyeLength.m
│   │   ├── depletionWidth.m
│   │   ├── builtInPotential.m
│   │   ├── mobilityModel.m         #    Caughey-Thomas doping-dependent mobility
│   │   ├── diffusionCoeff.m
│   │   ├── diffusionLength.m
│   │   ├── sheetCarrierDensity.m
│   │   ├── hallCoefficient.m
│   │   ├── thermalVelocity.m
│   │   └── dosEffectiveMass.m
│   ├── +thinFilm/                  # ✅ 7 functions — Deposition rate, Kiessig, Stoney, thermal
│   │   ├── depositionRate.m
│   │   ├── kiessigThickness.m
│   │   ├── stoneyStress.m
│   │   ├── doseFromCurrent.m
│   │   ├── thermalMismatchStrain.m
│   │   ├── multilayerThermalConductivity.m
│   │   └── diffusionLength_thermal.m
│   ├── +magnetic/                  # ✅ 4 functions — Magnetometry calculations
│   │   ├── momentPerAtom.m
│   │   ├── magnetization.m
│   │   ├── bohrMagnetonConvert.m
│   │   └── demagFactor.m           #    sphere/thin_film/cylinder/prolate/oblate
│   └── +substrates/                # ✅ 2 functions — 14 substrate presets
│       ├── getSubstrate.m          #    Lookup by name (Si, Al2O3, SrTiO3, MgO, etc.)
│       └── listSubstrates.m
├── tests/
│   ├── test_calc_units.m           # ✅ 85 tests — Phase 1 (constants, unit converter, element data)
│   ├── test_calc_modules.m         # ✅ 67 tests — Phase 2 (crystal, electrical, semiconductor, etc.)
│   └── test_materials_calc_gui.m   # [Phase 3] Headless GUI API tests
```

#### Phase 4: X-ray/Neutron integration (into DataPlotter)
...
> **Note:** Integrated into `DataPlotter` as sub-dialogs (SLD calculator,
...
~1500-2000 lines — smaller than DataPlotter (~11000) or emViewerGUI
...
2. **`DataPlotter.m`** — Phase 4 adds `calc.xrayNeutron` sub-dialogs (SLD
...
5. **Common wavelengths** — the GUI stores a wavelength preset dropdown
   (Cu Ka1, Mo Ka1, etc.). The same values are used in `DataPlotter`
...
- [x] Create `+calc/elementData.m` — periodic table database (118 elements,
   18 properties, neutron scattering lengths, X-ray edges).
- [x] Create `tests/test_calc_units.m` — 85 tests covering constants, unit scaling, compound units, temperature, bridges, element data. All passing.
...
- [x] Create `+calc/+substrates/` — getSubstrate, listSubstrates with 14
   presets (including dielectric constants).
- [x] Create `tests/test_calc_modules.m` — 67 tests covering all Phase 2 modules. All passing.
...
- [ ] Create `materialsCalcGUI.m` — figure, root grid, tab group, status bar,
   API struct.
- [ ] Build Unit Converter tab (free-text parser, swap, presets, live update).
- [ ] Build Crystal, Electrical, Semiconductor, Thin Film tabs (calculation
   cards with Calculate buttons).
- [ ] Build Periodic Table tab (element grid, property/color dropdowns,
   highlight search, detail bar).
- [ ] Add history log + copy-as-LaTeX on result displays.
- [ ] Create `tests/test_materials_calc_gui.m` — headless API tests.

### Phase 4: X-ray/Neutron integration (4 steps)

**Purpose:** Add scattering and diffraction calculations. These integrate
into DataPlotter (where the data lives) rather than the standalone
calculator.
...
dialog (`onFitReflectivity` in `DataPlotter.m`).
...
DataPlotter reflectivity FFT dialog.
