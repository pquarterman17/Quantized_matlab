...
### 1.2 Fitting dialog: nested function in `dataImportGUI.m`
...
### 2.1 Data table panel in `dataImportGUI.m`
...
| `dataImportGUI.m` | Template dropdown + save/apply buttons |
...
Popup dialog (nested function in `dataImportGUI.m`) showing:
...
| `dataImportGUI.m` | Batch fit dialog + parameter trend plot |
...
- **GUI is monolithic** — new dialogs are nested functions in
  `dataImportGUI.m`. Consider whether the file (already ~11k lines) needs
