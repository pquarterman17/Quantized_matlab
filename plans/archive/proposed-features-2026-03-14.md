...
**Touches:** `dataImportGUI.m` (new button + callback), dataset struct (synthetic flag).
...
**Touches:** New `+data/peak_database.mat`, `dataImportGUI.m` (peak panel button + overlay logic).
...
**Touches:** `dataImportGUI.m` (record toggle + script generation), programmatic API
...
**Touches:** `dataImportGUI.m` (dialog + evaluator), `+utilities/` (safe expression parser).
...
**Touches:** `dataImportGUI.m` (one button + 3-line callback).
...
**Touches:** `dataImportGUI.m` (`onMouseHover` callback, ~15 lines).
...
**Touches:** `dataImportGUI.m` (2 buttons + swap logic in `appData.datasets`).
...
**Touches:** `+parser/importQDVSM.m` (header parsing, ~20 lines), `dataImportGUI.m`
(auto-label logic).
...
**Touches:** `dataImportGUI.m` (fit engine, peak table column), `+utilities/pseudoVoigt.m`
...
**Touches:** New `+utilities/splitPearsonVII.m`, `dataImportGUI.m` (fit engine, dropdown,
peak table).
...
**Touches:** `dataImportGUI.m` (2D panel button + polar rendering), possibly new
...
**Touches:** `dataImportGUI.m` (playback UI + timer callbacks), `+plotting/` (optional
GIF export helper).
...
**Touches:** `dataImportGUI.m` (one new nested function + one `fig.KeyPressFcn` assignment).
...
**Touches:** New `+styles/dark.m`, `dataImportGUI.m` (theme dropdown + recursive updater).
...
**Touches:** `dataImportGUI.m` (`drawToAxes`, ~10 lines).

---

## Recently Implemented (2026-03-14)

The following 23 features were implemented in the dataImportGUI in a single session:
