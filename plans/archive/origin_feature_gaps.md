...
- `BosonPlotter.m` — added SG to smoothing dropdown, added "Batch Apply..." button to Templates dialog with `doBatchApplyTemplate()` nested function
- `+scripts/applyAnalysisTemplate.m` — NEW file, batch-applies saved BosonPlotter templates to folders of files (import → correct → export CSV + optional peaks)
...
**Status:** Added to `+utilities/smoothData.m` and BosonPlotter GUI dropdown.
...
**Status:** Added `+scripts/applyAnalysisTemplate.m` and extended BosonPlotter template system.
...
- Refactored `+bosonPlotter/curveFitting.m` dialog: category/model dropdowns, bounds, fixed params, weights, simulate, auto-guess, error display, chi²/AIC stats
...
- GUI integration: not yet wired into BosonPlotter corrections panel
...
**Status:** Implemented as `+bosonPlotter/roiAnalysis.m` (2026-03-21).

**Implementation:**
- `+bosonPlotter/roiAnalysis.m` — Interactive ROI gadget dialog
