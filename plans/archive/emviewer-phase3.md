# Fermion Feature Plan — Phase 3 (DM Parity)
...
instrument-tied features outside Fermion's scope as an offline viewer.
...
**Touches:** `Fermion.m` (button in Process section + callback), `+imaging/radialProfile.m` (new).
...
**Touches:** `Fermion.m` (new capture mode `'dspacing'`), measurement table integration.
...
**Touches:** `Fermion.m` (buttons in Process section), `+imaging/morphOp.m` (new).
...
**Touches:** `Fermion.m` (dropdown in Contrast section), `displayImage()` pipeline.
...
**Touches:** `Fermion.m` (new capture mode `'arrow'`, annotation struct extension).
...
**Touches:** `Fermion.m` (button + callback). ~15 lines.
...
**Touches:** `Fermion.m` (button), `+imaging/unsharpMask.m` (new). ~40 lines.
...
**Touches:** `Fermion.m` (width spinner in profile dialog), `+imaging/lineProfile.m` (modify). ~50 lines.
...
**Touches:** `Fermion.m` (new capture mode `'roiellipse'`). ~80 lines.
...
**Touches:** `Fermion.m` (button), `+imaging/binImage.m` (new). ~30 lines.
...
**Touches:** `+parser/importSER.m` (new), `+parser/resolveParser.m`, `Fermion.m` file filter.
...
**Touches:** `Fermion.m` (dialog), `+imaging/butterworthFilter.m` (new). ~60 lines.
...
**Touches:** `Fermion.m` (all processing callbacks that change dimensions).
...
**Touches:** `Fermion.m`, `+imaging/azimuthalIntegrate.m` (new). ~150 lines.
...
**Touches:** `Fermion.m` (new capture mode `'roipoly'`). ~90 lines.
...
**Touches:** `Fermion.m` (annotation struct extension). ~80 lines.
...
**Touches:** `Fermion.m` (menu item), leverages existing parsers + `imwrite`. ~60 lines.
...
**Touches:** `Fermion.m` (preferences extension). ~100 lines.
...
**Touches:** `Fermion.m` (button + new figure creation). ~40 lines.

---

## Features Intentionally Excluded

These DM features are outside Fermion's scope as an offline viewer:
