...
% ── Programmatic API ──────────────────────────────────────────────────────
%
%   api = Boson() returns a struct of function handles for
...
%   api.undoMgr()              — return the live boson.UndoManager instance
...
%   xrdConvertGUI, test_gui_harness
    ...
    % Construction delegated to +boson/buildPeakWindow.m.
    ...
    pw = boson.buildPeakWindow(appData, peakConsts);

    % Destructure peak window handles into the Boson closure
    peakFig           = pw.peakFig;
    ...
    % Construction delegated to +boson/buildMap2DPanel.m.
    % Callbacks are wired below after the struct is returned.
    mw = boson.buildMap2DPanel(analysisGL, ismac);
    mw.map2DPanel.Layout.Row = [1 2]; mw.map2DPanel.Layout.Column = 3;

    % Destructure 2D map handles into the Boson closure
    ...
    %ONADDFILES  Open a multi-select file dialog; load every chosen file.
        ...
        [fnames, fpath] = uigetfile( ...
            {'*.dat;*.csv;*.tsv;*.txt;*.xlsx;*.xls;*.xlsm;*.xlsb;*.ods;*.raw;*.brml;*.xrdml;*.refl;*.pnr;*.datA;*.datB;*.datC;*.datD;*.data;*.datb;*.datc;*.datd;*.jpg;*.jpeg;*.png;*.bmp;*.gif;*.tif;*.tiff;*.bcf;*.dm3;*.dm4;*.mrc;*.mrcs;*.ser;*.spm;*.000;*.001', ...
             'Supported data files (*.dat, *.csv, *.xlsx, *.raw, *.brml, *.xrdml, *.refl, *.pnr, *.datA/B/C/D, *.jpg, *.png, *.tif, *.bcf, *.dm3/4, *.mrc, *.ser, *.spm)'; ...
             '*.*','All files (*.*)'}, ...
    ...
    %  cell array of char vectors — normalise to a cell array before processing.
        try
            ...
            fprintf(2, '[Boson] DropFcn error: %s\n', ME.message);
    ...
    %SAVESESSIONDIRECT  Save session to .mat file (no dialog).
        ...
        guiState = boson.sessionManager.collectGuiState(widgets);
        boson.sessionManager.save(outPath, appData, guiState);
    end
    ...
    %LOADSESSIONDIRECT  Load session from .mat file (no dialog).
        ...
        [datasets, restored] = boson.sessionManager.load(matPath);
        ...
        widgets = buildSessionWidgets_();
        boson.sessionManager.applyGuiState(restored.guiState, widgets);

        rebuildDatasetList(true);
        if ~isempty(appData.datasets)
            boson.sessionManager.applyAxisSelections(restored.guiState, widgets);
            updateControlsForActiveDataset();
        end
        onPlot([],[]);
    end
    ...
    %ONREFINELATTICE  Open a dialog to assign hkl indices and refine lattice parameters.
        ...
        result = boson.peakTools.refineLattice(ds, wl_A, ...
    ...
    %ONMATCHPHASES  Thin wrapper — delegates to boson.peakTools.matchPhases.
        if isempty(appData.datasets) || appData.activeIdx < 1
            uialert(fig, 'Load a file first.', 'No data'); return;
        end
        ds   = appData.datasets{appData.activeIdx};
        ...
        try
            boson.peakTools.matchPhases(ds, wl_A, ...
    ...
    %ONFFTTHICKNESS  Thin wrapper — delegates to boson.peakTools.fftThickness.
        if isempty(appData.datasets) || appData.activeIdx < 1
            uialert(fig, 'Load a file first.', 'No data'); return;
        end
        ds   = appData.datasets{appData.activeIdx};
        ...
        result = boson.peakTools.fftThickness(ds, wl_A, ...
            ParentFig=fig, StatusFcn=@setStatus, ...
    ...
    %ONWILLIAMSONHALLPLOT  Williamson-Hall analysis: β·cosθ vs 4·sinθ.
        ...
        result = boson.peakTools.williamsonHall(ds, wl_A, ...
    ...
    %   btnColor  — [1×3] background colour for buttons
        ...
        if isempty(config)
            config = boson.toolbarDefaultConfig();
        end
    ...
    %ONCUSTOMISETOOLBAR  Open the toolbar customisation dialog and rebuild on OK.
        ...
        newCfg = boson.toolbarConfig(appData.toolbarConfig, regForDlg);
        ...
        if isfield(ds, 'parserName') && isNeutronParser(ds.parserName)
            ...
            for pki = 1:numel(appData.datasets)
                ...
                pUiVals = struct('xOff', xOff, 'yOff', yOff, ...
                    ...
                    'fieldUnit', 'Oe (raw)', 'momentUnit', 'emu (raw)');
                pParams = boson.correctionParams(pds, pUiVals);
                pCorr = boson.applyCorrections(pds.data, pParams);
                pds.corrData   = pCorr;
                pds.xOff       = xOff;
                pds.yOff       = yOff;
                pds.bgSlope    = 0;
    ...
    %   Called by onPlot (GUI uiaxes) and onExportFigure (regular axes).
        try
            ...
            % Extracted to +boson/drawOverlays.m for maintainability.
            ...
            if appData.activeIdx >= 1 && ~isempty(appData.datasets)
                boson.drawOverlays(targetAx, appData.datasets{appData.activeIdx}, ...
    ...
    %ONFITSURFACE  Open the 2D surface fitting dialog for the active map dataset.
        ...
        boson.surfaceFitDialog(mapData, Title="Surface Fit — " + ds.name);
    end
    ...
    %   The extracted profile is added as a new dataset in appData.datasets.
        ...
        updateControlsForActiveDataset();
        fprintf('[Boson] Line-cut added: %s — %s\n', [fn fext], cutLabel);
    end
    ...
    %     .mode        — 'Sum' or 'Mean'
        ...
        fprintf('[Boson] Arc integral added: %s — %s\n', [fn fext], cutLabel);
    end
    ...
    %  boson.sessionManager.collectGuiState / applyGuiState.
    ...
    %  Delegates serialisation to boson.sessionManager.save().
        if isempty(appData.datasets)
    ...
    %  Delegates deserialisation to boson.sessionManager.load().
        startDir = guiTernary(isempty(appData.lastDir), pwd, appData.lastDir);
    ...
    %   Calls boson.filterRows() with the current data struct.
    ...
    %   An empty expression clears the filter without touching manual masks.
        ...
        if isempty(expr)
            ...
            try
                passMask = boson.filterRows(d, expr);
    ...
    %ONOPENADVANCEDPEAKANALYSIS  Launch the advanced peak analysis dialog.
        ...
        boson.peakAnalysis(appData.datasets, appData.activeIdx, ax, ...
            'StatusFcn', @setStatus, ...
    ...
    %ONOPENCURVEFITDIALOG  Open general-purpose curve fitting dialog.
        ...
        boson.curveFitting(appData.datasets, appData.activeIdx, ax, ...
            'StatusFcn', @setStatus, ...
            'ButtonColors', struct('primary', BTN_PRIMARY, 'tool', BTN_TOOL, 'fg', BTN_FG));
    end
    ...
    %ONOPENHYSTERESISDIALOG  Open hysteresis loop analysis dialog.
        ...
        boson.hysteresisDialog(appData.datasets, appData.activeIdx, ax, ...
            'StatusFcn', @setStatus, ...
            'ButtonColors', struct('primary', BTN_PRIMARY, 'tool', BTN_TOOL, 'fg', BTN_FG));
    end
    ...
    %ONROIANALYSIS  Open ROI selection and statistics dialog.
        ...
        boson.roiAnalysis(appData.datasets, appData.activeIdx, ax, ...
            'StatusFcn', @setStatus);
    end
    ...
    %ONOPENREFLFITDIALOG  Open reflectivity fitting dialog (Parratt recursion).
        ...
        boson.reflFitting(appData.datasets, appData.activeIdx, ax, ...
            'StatusFcn', @setStatus, ...
            'ButtonColors', struct('primary', BTN_PRIMARY, 'tool', BTN_TOOL, 'fg', BTN_FG));
    end
...
%{
        ...
        %ONCFPLOTONMAIN  Overlay the fit curve on the main Boson axes.
        ...
        %CFPICKXRANGE  Click on Boson axes to set X min or max.
            cfFig.Visible = 'off';
            fig.Pointer = 'crosshair';
...
%}
    ...
    %ONOPENDIGITIZER  Delegates to boson.graphDigitizer.
        boson.graphDigitizer( ...
    ...
    %ONADVANCEDFIGUREBUILDER  Delegates to boson.figureBuilder.
        if isempty(appData.datasets) || appData.activeIdx < 1
            uialert(fig,'Load at least one file first.','No data'); return;
        end
        boson.figureBuilder(appData.datasets, appData.activeIdx, ...
            'ButtonColors', struct('primary', BTN_PRIMARY, 'tool', BTN_TOOL, 'fg', BTN_FG));
...
%   Delegates to boson.colorMaps for the actual implementation.
    colors = boson.colorMaps(colormapName, nColors);
end
...
%   Delegates to boson.safeEvalMathExpr for the actual implementation.
    result = boson.safeEvalMathExpr(expr, vars);
end
