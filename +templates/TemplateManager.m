...
%            templates.MetadataEditor
    ...
    function onEdit(~, ~)
        ...
        if ~isempty(result)
            ...
            refreshList();
            bosonPlotter.quietAlert(fig, sprintf('Template "%s" updated.', t.name), 'Saved', ...
                'Icon', 'success');
        end
    end
    ...
    function onImport(~, ~)
        ...
        try
            ...
            bosonPlotter.quietAlert(fig, sprintf('Could not parse JSON:\n%s', ME.message), ...
                'Import Error', 'Icon', 'error');
            return;
        end

        if ~isfield(t, 'name') || isempty(t.name)
            bosonPlotter.quietAlert(fig, 'JSON file is missing a "name" field.', ...
                'Import Error', 'Icon', 'error');
            return;
        end
        if ~isfield(t, 'type') || isempty(t.type)
            bosonPlotter.quietAlert(fig, 'JSON file is missing a "type" field.', ...
                'Import Error', 'Icon', 'error');
            return;
        end
        ...
        refreshList();
        bosonPlotter.quietAlert(fig, sprintf('Imported "%s" as a user template.', t.name), ...
    ...
    function onExport(~, ~)
        ...
        bosonPlotter.quietAlert(fig, sprintf('Exported to:\n%s', outPath), 'Exported', ...
            'Icon', 'success');
    end
