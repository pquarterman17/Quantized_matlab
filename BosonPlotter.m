...
%   xrdConvertGUI, test_gui_harness
    ...
    menuCb = struct( ...
        ...
        'onPlotTemplates',@(~,~)anaCb.onPlotTemplates([],[]), 'onPublishFigure',@onPublishFigure, 'onOpenPlotStyleDialog',@onOpenPlotStyleDialog, 'onAdvancedFigureBuilder',@(~,~)anaCb.onAdvancedFigureBuilder([],[]), ...
        'onComposeFigure',@onComposeFigure, 'onLayout1x2',@(~,~)figDocDispatch_('layout1x2'), 'onLayout2x1',@(~,~)figDocDispatch_('layout2x1'), 'onLayout2x2',@(~,~)figDocDispatch_('layout2x2'), 'onBatchFigureExport',@(~,~)anaCb.onBatchFigureExport([],[]), 'onPolarPlot',@onPolarPlot, ...
    ...
    ddTemplate = uidropdown(styleGL, 'Items', tplItems, ...
        ...
        'Tooltip', ['Visual template (font / line width / marker size / ' ...
                    ...
                    'aps, nature, thesis, presentation, poster, report, web.  ' ...
                    'User templates appear with a "user:" prefix.'], ...
    ...
    % Rows 6–9 + preview axes panel — extracted to +bosonPlotter/buildPlotControlsPanel.m
    plotCtrlCb_ = struct( ...
        'onPlot',                 @onPlot, ...
        'onAxisChanged',          @onAxisChanged, ...
        'onWaterfallToggled',     @onWaterfallToggled, ...
        'onAutoLimits',           @onAutoLimits, ...
        'onSmartScale',           @onSmartScale, ...
        'onAnnotationModeChanged',@onAnnotationModeChanged, ...
        'onOpenPlotStyleDialog',  @onOpenPlotStyleDialog, ...
        'onShowPlotOptionsMenu',  @onShowPlotOptionsMenu);
    plotCtrlS_ = bosonPlotter.buildPlotControlsPanel( ...
        ctrlGL, contentGL, tk, struct(), plotCtrlCb_);
    % Unpack closure handles
    limGL              = plotCtrlS_.limGL;
    efXMin             = plotCtrlS_.efXMin;
    efXMax             = plotCtrlS_.efXMax;
    ddXFmt             = plotCtrlS_.ddXFmt;
    efYMin             = plotCtrlS_.efYMin;
    efYMax             = plotCtrlS_.efYMax;
    ddYFmt             = plotCtrlS_.ddYFmt;
    efY2Min            = plotCtrlS_.efY2Min;
    efY2Max            = plotCtrlS_.efY2Max;
    ddY2Fmt            = plotCtrlS_.ddY2Fmt;
    cbWaterfall        = plotCtrlS_.cbWaterfall;
    efWaterfallSpacing = plotCtrlS_.efWaterfallSpacing;
    cbCountsPerSec     = plotCtrlS_.cbCountsPerSec;
    cbAnnotationMode   = plotCtrlS_.cbAnnotationMode;
    ...
    % The toolbar is built/rebuilt by buildToolbar().  Column widths are set
    % dynamically; the single-row grid is created inside buildPlotControlsPanel.
    ...
    tbActions(end+1) = struct('id','cursor',         'label','Cursor', ...
        ...
        'callback',@(~,~) onToggleDataCursor([],[]), 'iconOnly',true,  'group','navigate');
    tbActions(end+1) = struct('id','autoscale',      'label','Auto', ...
        'tooltip','Reset all axis limits to auto-scale', ...
        'callback',@(~,~) onAutoLimits([],[]),       'iconOnly',true,  'group','navigate');
    ...
    tbActions(end+1) = struct('id','reexport',       'label','Re-export', ...
        'tooltip','Repeat last export (same profile + path)', ...
        'callback',@(~,~) figDocDispatch_('reexport'), 'iconOnly',false, 'group','output');
    tbActions(end+1) = struct('id','figBuilder',     'label','Figure Builder', ...
        'tooltip','Open the Figure Builder for publication-quality figures', ...
        'callback',@(~,~) onAdvancedFigureBuilder([],[]), 'iconOnly',false, 'group','analyze');
    tbActions(end+1) = struct('id','animate',        'label','Animate', ...
        'tooltip','Animate dataset sequence', ...
        'callback',@(~,~) onToggleAnimation([],[]),  'iconOnly',false, 'group','analyze');
    tbActions(end+1) = struct('id','workspace',      'label','Data Table', ...
        ...
        'callback',@(~,~) openLinkedDataWorkspace(), 'iconOnly',false, 'group','analyze');
    tbActions(end+1) = struct('id','watchFile',      'label','Watch', ...
        ...
        'callback',@(~,~) onToggleWatchFile(),       'iconOnly',false, 'group','analyze');
    ...
    tbActions(end+1) = struct('id','peakWorkshop',  'label','Peaks', ...
        ...
        'callback',@(~,~) showPeakWindow(),          'iconOnly',false, 'group','analyze');
    ...
    appData.prefixFactors = [1,     1e12,      1e9,       1e6,           1e3, ...
                             1e-3,  1e-6,      1e-9];
    try
        cmCb.onSetAxisPrefixFromMenu  = @onSetAxisPrefixFromMenu;
        cmCb.onContextToggle          = @onContextToggle;
        cmCb.onAddRefLineAtCursor     = @onAddRefLineAtCursor;
        cmCb.onAddHRefLine            = @onAddHRefLine;
        cmCb.onAddVRefLine            = @onAddVRefLine;
        cmCb.onClearRefLines          = @onClearRefLines;
        cmCb.onToggleDataCursor       = @onToggleDataCursor;
        cmCb.onEditAxisLabelsMenu     = @onEditAxisLabelsMenu;
        cmCb.onOpenLegendEditor       = @onOpenLegendEditor;
        cmCb.onAutoLimits             = @onAutoLimits;
        cmCb.onSetAxisLimitsMenu      = @onSetAxisLimitsMenu;
        ...
        cmCb.onClearFitOverlays       = @onClearFitOverlays;
        cmCb.refreshState             = @refreshState;
        cmCb.onCreateInsetFromMenu    = @onCreateInsetFromMenu;
        cmCb.onRemoveInset            = @onRemoveInset;
        cmCb.onCustomiseToolbar       = @onCustomiseToolbar;
        ax.ContextMenu = bosonPlotter.buildAxesContextMenu(fig, ...
            appData.prefixNames, DS_COLOR_NAMES, cmCb);
    catch
    ...
    analysisGL = uigridlayout(analysisPanel,[2 4], ...
        'ColumnWidth', {357, '1x', 0, 230}, ...   % col4=Save/Export visible at startup (3ec503f had zeroed it, hiding the panel until a dataset loaded)
        'RowHeight',   {110, '1x'}, ...
    ...
    tblPalette_ = struct('export', BTN_EXPORT, 'danger', BTN_DANGER, ...
        'primary', BTN_PRIMARY, 'tool', BTN_TOOL, 'fg', BTN_FG);
    tblCbs_ = struct('onOpenDataWorkspace', @(~,~) openLinkedDataWorkspace());
    dtUI_ = bosonPlotter.buildDataTablePanel(analysisGL, fig, tk, tblPalette_, tblCbs_);
    dataTablePanel    = dtUI_.dataTablePanel;
    tblData           = dtUI_.tblData;
    tblUnits          = dtUI_.tblUnits;
    lblTableStats     = dtUI_.lblTableStats;
    lblTableUnits     = dtUI_.lblTableUnits;
    efFilter          = dtUI_.efFilter;
    btnTableSaveAs    = dtUI_.btnTableSaveAs;
    btnTableMask      = dtUI_.btnTableMask;
    btnTableUnmask    = dtUI_.btnTableUnmask;
    btnTableDescStats = dtUI_.btnTableDescStats;
    btnSortAsc        = dtUI_.btnSortAsc;
    btnSortDesc       = dtUI_.btnSortDesc;
    btnFilterApply    = dtUI_.btnFilterApply;
    btnFilterClear    = dtUI_.btnFilterClear;
    ...
    exportCb_ = struct( ...
        ...
        'onAdvancedFigureBuilder',@(~,~)anaCb.onAdvancedFigureBuilder([],[]), ...
        'onPlotTemplates',        @(~,~)anaCb.onPlotTemplates([],[]), ...
        'onOverlayModeChanged',   @(~,~)anaCb.onOverlayModeChanged([],[]), ...
        'onBatchFigureExport',    @(~,~)anaCb.onBatchFigureExport([],[]), ...
        'onShowPlotOptionsMenu',  @onShowPlotOptionsMenu);
    ...
    btnBatchConvertXRD    = expUI_.btnBatchConvertXRD; %#ok<NASGU>
    ...
    %ONADDFILES  Open a multi-select file dialog; load every chosen file.
        ...
        [fnames, fpath] = uigetfile( ...
            {'*.dat;*.csv;*.tsv;*.txt;*.xlsx;*.xls;*.xlsm;*.xlsb;*.ods;*.raw;*.brml;*.xrdml;*.refl;*.pnr;*.datA;*.datB;*.datC;*.datD;*.data;*.datb;*.datc;*.datd;*.spm;*.000;*.001', ...
    ...
    %BUILDTOOLBAR  Delegate to module; group captions use themed textMuted (palette-safe).
        if nargin < 5, iconColor = []; end
        capCol = []; try, capCol = bosonPlotter.uxTokens(lower(appData.theme)).color.textMuted; catch, end
        bosonPlotter.buildToolbar(parentGL, config, registry, btnColor, iconColor, capCol);
    end
...
function onSendToOrigin(~,~)
    ...
    function onPublishFigure(~,~)
        pubCb.drawToAxes  = @drawToAxes;
        pubCb.setStatus   = @setStatus;
        pubCb.logGUIError = @logGUIError;
    ...
    %ONSAVEASTEMPLATE  Save the active dataset's column layout as a template.
        bosonPlotter.saveAsTemplate(appData, fig, @setStatus);
    end
