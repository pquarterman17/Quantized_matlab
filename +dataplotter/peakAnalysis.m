...
%
%   dataplotter.peakAnalysis(datasets, activeIdx, mainAx)
%
...
%
%   Results are communicated back to DataPlotter via PeakUpdateCallback.
...
if isempty(datasets) || activeIdx < 1 || activeIdx > numel(datasets)
    error('dataplotter:peakAnalysis:noDataset', 'No valid dataset.');
end
...
if numel(xv) < 10
    error('dataplotter:peakAnalysis:tooFewPoints', 'Need at least 10 data points.');
end
...
row = 22;
btnApply = uibutton(ctrlGL, 'Text', 'Apply to DataPlotter', ...
...
% ════════════════════════════════════════════════════════════════════════
    ...
    function onApply()
        ...
        setStatusLocal(sprintf('Applied %d peaks to DataPlotter.', numel(peaksOut)));
