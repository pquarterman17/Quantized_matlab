...
%
%   Syntax:
%     mask = dataplotter.filterRows(dataStruct, expression)
%
%   Inputs:
...
%
%   Examples:
%     mask = dataplotter.filterRows(d, 'Temp > 300')
%     mask = dataplotter.filterRows(d, 'Field >= -500 & Field <= 500')
%     mask = dataplotter.filterRows(d, 'between(x, 0, 1e-3)')
%     mask = dataplotter.filterRows(d, '')   % all-true mask
...
if pos <= numel(tokens)
    error('dataplotter:filterRows:unexpectedToken', ...
...
% ════════════════════════════════════════════════════════════════════════
    ...
    function val = parseComparison()
        ...
        switch op
            ...
            case '~='
                val = lhs ~= rhs;
            otherwise
                error('dataplotter:filterRows:unknownOp', ...
    ...
    function val = parsePrimary()
        if pos > numel(tokens)
            error('dataplotter:filterRows:unexpectedEnd', ...
        ...
        if strcmp(tok, '(')
            ...
            val = parseOr();
            if pos > numel(tokens) || ~strcmp(tokens{pos}, ')')
                error('dataplotter:filterRows:missingParen', ...
        ...
        error('dataplotter:filterRows:unknownToken', ...
    ...
    function expectOpen()
        if pos > numel(tokens) || ~strcmp(tokens{pos}, '(')
            error('dataplotter:filterRows:missingOpenParen', ...
    ...
    function expectClose()
        if pos > numel(tokens) || ~strcmp(tokens{pos}, ')')
            error('dataplotter:filterRows:missingCloseParen', ...
    ...
    function expectComma()
        if pos > numel(tokens) || ~strcmp(tokens{pos}, ',')
            error('dataplotter:filterRows:missingComma', ...
...
while i <= n
    ...
    error('dataplotter:filterRows:badChar', ...
