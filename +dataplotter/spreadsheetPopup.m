...
% ── Syntax ────────────────────────────────────────────────────────────────
%
%   fig = dataplotter.spreadsheetPopup(dataStruct)
%   fig = dataplotter.spreadsheetPopup(dataStruct, Title=..., OnEdit=..., ReadOnly=...)
%
% ── Inputs ────────────────────────────────────────────────────────────────
%
...
%   dataplotter.spreadsheetPopup(d);
%   dataplotter.spreadsheetPopup(d, Title='My Dataset', ReadOnly=true);
%
% ════════════════════════════════════════════════════════════════════════
...
% ════════════════════════════════════════════════════════════════════════
    ...
    %ONFILTER  Apply the filter expression from the edit field.
        ...
        try
            mask = dataplotter.filterRows(dataStruct, expr);
    ...
    %ONCELLEDIT  Propagate cell edits through OnEdit callback.
        ...
        if ~isempty(Options.OnEdit) && isa(Options.OnEdit, 'function_handle')
            try
                ...
                warning('dataplotter:spreadsheetPopup:editCallbackError', ...
