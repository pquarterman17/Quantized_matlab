...
testDir = fullfile(tempdir, ['batch_xrd_conv_' char(datetime('now','Format','yyyyMMdd_HHmmss'))]);
mkdir(testDir);
cleanup_obj = onCleanup(@() rmdir(testDir, 's'));
...
try
    data = parser.importXRDML(XRDML_FILE);
...
try
    results = scripts.batchConvertXRD(XRDML_DIR, ...
        OutputDir=testDir, ...
        Verbose=false);
...
try
    ...
    results = scripts.batchConvertXRD(XRDML_FILE, ...
...
if failed > 0
    error('test_batch_xrd_converter:failures', '%d test(s) failed.', failed);
end
